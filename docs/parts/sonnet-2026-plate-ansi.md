---
title: 2026 Sonnet Plate / ANSI
---

`2022 Sonnet` `2024 Sonnet` `2026 Sonnet`

![2026 Sonnet Plate / ANSI](../assets/sonnet-2026-plate-ansi.webp)

## Availability

<div class="grid cards" markdown>

-   :material-store:{ .lg .middle } __Buy from Mode__

    ---

    Available for purchase directly from Mode.

    [:octicons-link-external-24: View product page](https://modedesigns.com/collections/all/products/sonnet-plate-2026){ target="_blank" rel="noopener" title="Buy from Mode (opens in new tab)" }

-   :material-hammer-wrench:{ .lg .middle } __Have it Made__

    ---

    Use the design files below to have the part manufactured.

    [:octicons-arrow-right-24: Design Files](#design-files){ title="Design Files" }

</div>

## Design Files

[:material-download: DXF](../files/sonnet-2026-plate-ansi.dxf){ download="sonnet-2026-plate-ansi.dxf" title="Download DXF" }

[:material-download: STEP](../files/sonnet-2026-plate-ansi.step){ download="sonnet-2026-plate-ansi.step" title="Download STEP" }
